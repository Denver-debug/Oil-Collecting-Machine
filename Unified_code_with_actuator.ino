/** ESP32 Unified Tank Controller with Linear Actuator
 * - Mobility: Dual BTS7960 motor drivers with circular joystick
 * - Auxiliary: Pump and Aux Motor via MOSFET modules
 * - Linear Actuator: BTS7960 motor driver with extend/retract buttons
 ...
 * 
 * LINEAR ACTUATOR WIRING (BTS7960):
 * Actuator: RPWM=23, LPWM=22, R_EN=21, L_EN=4
 * 
...
// ===== LINEAR ACTUATOR (BTS7960 Driver #3) =====
const int RPWM3_PIN = 23;
const int LPWM3_PIN = 22;
const int R_EN3_PIN = 21;
const int L_EN3_PIN = 4;

// ===== AUXILIARY DEVICES (MOSFET) =====
const int PUMP_PIN = 14;
const int AUX_MOTOR_PIN = 12;
...
int actuatorState = 0;  // 0=stopped, 1=extending, -1=retracting
...
void controlActuator(int direction) {
  digitalWrite(R_EN3_PIN, HIGH);
  digitalWrite(L_EN3_PIN, HIGH);
  
  if (direction == 0) {
    // Stop
    ledcWrite(RPWM3_PIN, 0);
    ledcWrite(LPWM3_PIN, 0);
    actuatorState = 0;
  } else if (direction > 0) {
    // Extend
    ledcWrite(LPWM3_PIN, 0);
    delayMicroseconds(200);
    ledcWrite(RPWM3_PIN, 200);
    actuatorState = 1;
  } else {
    // Retract
    ledcWrite(RPWM3_PIN, 0);
    delayMicroseconds(200);
    ledcWrite(LPWM3_PIN, 200);
    actuatorState = -1;
  }
}
  ...
  <div class="aux-section">
    ...
    <div class="aux-controls">
      ...
      <div class="aux-card">
        ...
        <button class="toggle-button off" id="pumpBtn" onclick="togglePump()">⏻</button>
      </div>
      ...
      <div class="aux-card">
        ...
        <button class="toggle-button off" id="auxBtn" onclick="toggleAux()">⏻</button>
      </div>
      ...
      <div class="aux-card">
        <div class="aux-info">
          <div class="aux-icon">↕️</div>
          <div class="aux-details">
            <div class="aux-name">Linear Actuator</div>
            <div class="aux-status" id="actuatorStatus">STOPPED</div>
          </div>
        </div>
        <div class="actuator-buttons">
          <button class="toggle-button off" id="extendBtn" 
                  onmousedown="actuatorExtend()" 
                  onmouseup="actuatorStop()"
                  ontouchstart="actuatorExtend()" 
                  ontouchend="actuatorStop()">
            ▲
          </button>
          <button class="toggle-button off" id="retractBtn" 
                  onmousedown="actuatorRetract()" 
                  onmouseup="actuatorStop()"
                  ontouchstart="actuatorRetract()" 
                  ontouchend="actuatorStop()">
            ▼
          </button>
        </div>
    ...
    <button class="emergency-stop" onclick="emergencyStop()">⚠️ EMERGENCY STOP</button>
  </div>
</div>
...
function actuatorExtend() {
  fetch('/actuator/extend').catch(() => {});
}

function actuatorRetract() {
  fetch('/actuator/retract').catch(() => {});
}

function actuatorStop() {
  fetch('/actuator/stop').catch(() => {});
}
  ...
  fetch('/actuator/status')
    ...
    .then(data => {
      ...
      if (actuatorState === 1) {
        ...
        extendBtn.className = 'toggle-button on';
        retractBtn.className = 'toggle-button off';
      } else if (actuatorState === -1) {
        status.textContent = 'RETRACTING';
        extendBtn.className = 'toggle-button off';
        retractBtn.className = 'toggle-button on';
      } else {
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n\n=== ESP32 Unified Tank Controller with Actuator ===");
  ...
  pinMode(R_EN3_PIN, OUTPUT);
  pinMode(L_EN3_PIN, OUTPUT);
  digitalWrite(R_EN3_PIN, LOW);
  digitalWrite(L_EN3_PIN, LOW);
  ...
  // Setup PWM for actuator
  ledcAttach(RPWM3_PIN, PWM_FREQ, PWM_RESOLUTION);
  ledcAttach(LPWM3_PIN, PWM_FREQ, PWM_RESOLUTION);
  ledcWrite(RPWM3_PIN, 0);
  ledcWrite(LPWM3_PIN, 0);
  ...
  server.on("/actuator/extend", handleActuatorExtend);
  server.on("/actuator/retract", handleActuatorRetract);
  server.on("/actuator/stop", handleActuatorStop);
  server.on("/actuator/status", handleActuatorStatus);
